...
from IIIFingest.settings import MPS_MANIFEST_BASE_URL, MPS_ASSET_BASE_URL
...
class TestClient:
    ...
    def test_client_upload_asset_id(self, test_images, boto_session, test_client):
        boto_session.resource('s3').create_bucket(Bucket=self.bucket_name)
        ...
        image_dict = assets[0]
        assert image_dict.filepath == image_path
        assert image_dict.asset_id.startswith(client.asset_prefix + test_image_id) == True

    def test_client_fail_upload_non_alphanumeric_asset_id(self, test_images, boto_session, test_client):
        boto_session.resource('s3').create_bucket(Bucket=self.bucket_name)
    ...
    def test_client_fail_upload_nonexistent_bucket(self, test_images, boto_session, test_client):
    ...
    def test_client_create_manifest(self, test_images, boto_session, test_client):
        ...
        assert manifest["id"].startswith(manually_created_base_manifest_url) == True
        # Test base asset url is correctly created
        manifest_body = item["items"][0]["items"][0]["body"]
        assert manifest_body["id"].endswith(manually_created_service) == True
        assert manifest_body["id"].startswith(manually_created_base_asset_url) == True
